	...
	<DetailsDiv>
		<IconsDiv>
			{icons.map((index, i) => {
				...
				return <IconDiv color={COLORS.GREY_2} key={i}><Svg /></IconDiv>;
			})}
		</IconsDiv>
		<StyledH6 weight={400} color={COLORS.GREY_1}>{title}</StyledH6>
			...
			<Section id={this.props.id} title='For Fun (Outdated)' transparent>
				<WrapperDiv>
					...
					<ShowAllButton onClick={this.showAll}>
						<TYPOGRAPHY.S1 weight={400} color='inherit'>{this.state.showAllText}</TYPOGRAPHY.S1>
					</ShowAllButton>
				</WrapperDiv>
			</Section>
		)
